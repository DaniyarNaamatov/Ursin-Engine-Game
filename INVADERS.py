...
def update():
    ...
    for invader in invaders:
        ...
        if invader.y < -0.55:
            ...
            player.y = 10
            Text(text='ТЫ ПРОИГРАЛ!', origin=(0, 0), scale=2, color=color.yellow, background=True)

    for bullet in bullets:
        bullet.y += time.dt * bullet.dy
        hit_info = bullet.intersects()
        if hit_info.hit:
            Audio('space/explosion_sound.wav')
            bullet.x = 10
            score += 1
            text.y = 10
...
field_size = 19
Entity(model='quad', scale=60, texture='space/blue_sky')
